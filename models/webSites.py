from sqlalchemy import Column, Integer, String, Boolean, Date, VARCHAR
...
class WebSites(Model):
    __tablename__ = TableNames.WebSites.value

    id = Column("id", Integer, primary_key=True, nullable=False, unique=True)
    domain_url = Column("domain_url", VARCHAR(50), nullable=False, unique=True)
    ...
    def __init__(self, id, domain_url: str,using_js, using_asp, using_php, is_support_ios, is_support_android, page_rank=None):
        self.id = id
